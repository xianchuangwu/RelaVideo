 ...
 * Description: TODO 音频转换成为aac编码 同时剪裁
 */

@RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
public class Mp3ToAACWithClip {
    ...
    public Mp3ToAACWithClip(String srcFile, String outFile, long rangeStart, long rangeEnd, OnProgressListener listener) {
    ...
    private class DecodeTask implements Runnable, IDataObtain {
        private static final long INPUT_TIME_OUT = -1;
        private static final long OUTPUT_TIME_OUT = 10000;
        ...
        public void setRangeTime(long rangeStart, long rangeEnd) {
            this.rangeStart = rangeStart;
            if (videoHasAudio) {
                this.rangeEnd = (long) ((double) rangeEnd * videoSampleRate / Constant.EncodeConfig.OUTPUT_AUDIO_SAMPLE_RATE_HZ);
            } else {
                this.rangeEnd = rangeEnd;
            }
        }
        ...
        @Override
        public void run() {
            Mp3ToAACWithClip.logMsg("decodec run");
            if (listener != null) {
                listener.onStart();
            }
            ...
            try {
                ...
                Mp3ToAACWithClip.logMsg("decode IOException :" + Log.getStackTraceString(e));
                e.printStackTrace();
            }
            Mp3ToAACWithClip.logMsg("decodec isPrepare  " + isPrepare);
            if (isPrepare) {
                decode();
            }
            release();
        ...
        private void prepare() throws IOException {
            ...
            for (int i = 0; i < numTracks; i++) {
                ...
                if (!TextUtils.isEmpty(mine) && mine.startsWith("audio")) {
                    ...
                    codec.start();
                    Mp3ToAACWithClip.logMsg("New decode codec start:" + format.toString());
                    break;
                }
            }
        ...
        private void decode() {
            ...
            Mp3ToAACWithClip.logMsg("inputBuffers length: " + inputBuffers.length);
            Mp3ToAACWithClip.logMsg("outputBuffers length: " + outputBuffers.length);
            MediaCodec.BufferInfo info = new MediaCodec.BufferInfo();
            Mp3ToAACWithClip.logMsg("loopDecode   start");
            ...
            while (true) {
                long timestamp = 0;
                if (!isEOS) {
                    int inIndex = codec.dequeueInputBuffer(INPUT_TIME_OUT);
                    if (inIndex >= 0) {
                        ByteBuffer buffer = inputBuffers[inIndex];
                        int sampleSize = extractor.readSampleData(buffer, 0);
                        long timestampTemp = extractor.getSampleTime();
                        timestamp = timestampTemp / 1000;
                        Mp3ToAACWithClip.logMsg("loopDecode  readSampleData end sampleSize  " + sampleSize + "    buffer.capacity()=" + buffer.capacity());
                        Mp3ToAACWithClip.logMsg("loopDecode  readSampleData end timestamp " + timestamp);
                        ...
                        if (sampleSize <= 0) {
                            ...
                            isEOS = true;
                            Mp3ToAACWithClip.logMsg("loopDecode  isEOS true ");
                        } else {
                            codec.queueInputBuffer(inIndex, 0, sampleSize, timestampTemp, 0);
                            extractor.advance();
                            Mp3ToAACWithClip.logMsg("loopDecode  extractor advance");
                        }
                    }
                }
                int outIndex = codec.dequeueOutputBuffer(info, OUTPUT_TIME_OUT);
                Mp3ToAACWithClip.logMsg(" switch (outIndex)");
                switch (outIndex) {
                    case MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED:
                        outputBuffers = codec.getOutputBuffers();
                        Mp3ToAACWithClip.logMsg("dequeueOutputBuffer INFO_OUTPUT_BUFFERS_CHANGED!");
                        break;
                    case MediaCodec.INFO_OUTPUT_FORMAT_CHANGED:
                        ...
                        int pcmEncoding;
                        try {
                            pcmEncoding = mf.getInteger(MediaFormat.KEY_PCM_ENCODING);
                        } catch (Exception e) {
                            Mp3ToAACWithClip.logMsg("pcmEncoding Exception :" + Log.getStackTraceString(e));
                        ...
                        Mp3ToAACWithClip.logMsg("New format " + mf.toString());
                        break;
                    case MediaCodec.INFO_TRY_AGAIN_LATER:
                        Mp3ToAACWithClip.logMsg("dequeueOutputBuffer timed out!");
                        break;
                    default:
                        if (last == 0) {
                        ...
                        Mp3ToAACWithClip.logMsg("解码时间：" + (now - last) + " info.size  " + info.size);
                        last = now;
                        ...
                        pushAvFrame(outData);
                        if (listener != null) {
                            listener.onProgress(rangeEnd > 0 ? (int) rangeEnd : (int) duration, rangeStart > 0 ? (int) (timestamp - rangeStart) : (int) timestamp);
                        }
                        break;
                }
                // All decoded frames have been rendered, we can stop playing now
                if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    Mp3ToAACWithClip.logMsg("OutputBuffer BUFFER_FLAG_END_OF_STREAM");
                    break;
                }
            }
        }
    ...
    private class EncodeTask implements Runnable {
        ...
        private void encode() {
            ...
            while (true) {
                ...
                if (outIndex >= 0) {
                    ...
                    long now = System.currentTimeMillis();
                    Mp3ToAACWithClip.logMsg("编码码时间：" + (now - last) + " info.size  " + info.size);
                    last = now;
                    while (outIndex >= 0) {
                ...
                if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                    Log.d(Mp3ToAACWithClip.class.getSimpleName(), "encode OutputBuffer BUFFER_FLAG_END_OF_STREAM");
                    break;
                }
            }
        }
        ...
        private void prepare() throws IOException {
            String mime = Constant.EncodeConfig.OUTPUT_AUDIO_MIME_TYPE;
            encoder = MediaCodec.createEncoderByType(mime);
            MediaFormat format = MediaFormat.createAudioFormat(mime, sampleRate, channelCount);
            format.setInteger(MediaFormat.KEY_BIT_RATE, Constant.EncodeConfig.OUTPUT_AUDIO_BIT_RATE);
            format.setInteger(MediaFormat.KEY_PCM_ENCODING, pcmEncoding);
            format.setInteger(MediaFormat.KEY_MAX_INPUT_SIZE, 20 * 1024);
            format.setInteger(MediaFormat.KEY_AAC_PROFILE, Constant.EncodeConfig.OUTPUT_AUDIO_AAC_PROFILE);
    ...
    private boolean createFile(String filePath, boolean recreate) {
